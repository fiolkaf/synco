...
function ObservableArray(array) {
    ...
    function getObservableArrayObject(index, item) {
        if (typeof item === 'object' && !item.hasOwnProperty('id')) {
            throw 'Please define id property for the item' + item;
        }
        ...
        var unsubscribe = observable.on('change', function(evt) {
            ...
            evt.key = '.' + (item.id ? item.id : item) + '.' + evt.key;
            proxy._trigger.call(this, 'change', evt);
        });
        proxy.addDisposable(unsubscribe);
        proxy.addDisposable(observable.dispose);
        observable.addDisposable(unsubscribe);
        return observable;
    }
...
function ObservableObject(data) {
    ...
    Object.getOwnPropertyNames(data).forEach(function(key) {
        ...
        proxy.addDisposable(unsubscribe);
        proxy.addDisposable(childProxy.dispose);
        childProxy.addDisposable(unsubscribe);
    });
    ...
    var unsubscribe = proxy.on('change', function(proxyEvt) {
        ...
        proxy.addDisposable(unsubscribe);
        proxy.addDisposable(object.dispose);
        object.addDisposable(unsubscribe);
    });
    proxy.addDisposable(unsubscribe);
